...
function [resonant_frequencies, mode_shapes] = cylindrical_resonant_modes(N, radius, height, speed_of_sound, BCr, BCz)
    ...
    for m = 0:N
        ...
        if BCr == 'D'
            radial_zeros = besselj_zeros(m, N);
        elseif BCr == 'N'
            radial_zeros = besselj_zeros(m+1, N);
        else
...
function bessel_zeros = besselj_zeros(nu, N)
    % Finds the first N zeros of the Bessel function J_nu(x)
    bessel_zeros = zeros(N, 1);
    for i = 1:N
        x_guess = (i - 0.5) * pi;
        bessel_zeros(i) = fzero(@(x) besselj(nu, x), x_guess);
    end
end
