...
def interrupt(instrument_deques):
    ...
    input_reading = float(input("What is the TEMP? (Numbers only) \n"))
    T.append(input_reading)    
...
if __name__ == "__main__":
    ...
    with open(filename, "a", newline='') as f:
        ...
        row = []
        for j in range(instrument_count):
            row.extend([f"Reading {j + 1} X", f"Reading {j + 1} Y", "Slope (m)", "Intercept (c)", "Temp (C)"])
